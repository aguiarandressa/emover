(function () {
  ...
  if (!buttonsRoot || !contentsRoot || !Array.isArray(tabsData)) return;

  buttonsRoot.innerHTML = tabsData
    .map(
      (tab, index) => `
    <button class="tab-btn flex flex-col items-center gap-3 p-6 rounded-2xl transition-all ${
      index === 0
  ...
  contentsRoot.innerHTML = tabsData
    .map(
      (tab, index) => `
    <div id="${tab.id}" class="tab-content ${index === 0 ? '' : 'hidden '}grid md:grid-cols-2 gap-12 items-center px-4 pb-4">
      <div class="order-2 md:order-1">
        ...
        <ul class="space-y-4">
          ${tab.bullets
            .map(
              (item) => `<li class="flex items-center gap-3"><span class="material-symbols-outlined text-primary bg-primary/10 p-1 rounded-full text-sm font-bold">check</span><span class="text-slate-700 dark:text-slate-300">${item}</span></li>`,
            )
            .join('')}
        </ul>
      </div>
